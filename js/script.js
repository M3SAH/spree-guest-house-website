document.addEventListener('DOMContentLoaded', () => {
    ...
    bookingForms.forEach((form) => {
        form.addEventListener('submit', (event) => {
            ...
            sendWhatsApp(`Hello Spree Guest House, I would like to check availability for ${room} from ${arrival} to ${departure} for ${guests}.`);
        });
    });
    ...
    inquiryForms.forEach((form) => {
        form.addEventListener('submit', (event) => {
            event.preventDefault();
            const data = new FormData(form);
            const subject = encodeURIComponent(`Spree Guest House inquiry - ${data.get('purpose') || 'Inquiry'}`);
            const body = encodeURIComponent(
                `Name: ${data.get('name') || ''}\nPhone: ${data.get('phone') || ''}\nEmail: ${data.get('email') || ''}\nPurpose: ${data.get('purpose') || ''}\n\nMessage:\n${data.get('message') || ''}`
            );
            window.location.href = `mailto:spreeguesthouse@yahoo.com?subject=${subject}&body=${body}`;
            showToast('Opening your email app with the inquiry details.');
        });
    });
